...
1. Download the most recent completed Tree Nine run's (heneforth "last run") combined diff file (bigtree_combinedYYYY-MM-DD.diff) and samples added file (samples_added_YYYY-MM-DD), as well as set data table zip (whatever_set.zip)
...
3. `python3 redact_samples.py denylist.txt --samples_added_file samples_added_YYYY-MM-DD --combined_diff_file bigtree_combined_YYYY-MM-DD.diff` will create MODIFIED_bigtree_combinedYYYY-MM-DD.diff and MODIFIED_samples_added_YYYY-MM-DD in your workdir
4. Unzip whatever_set.zip to get `whatever_set/whatever_set_membership.tsv` and `whatever_set/whatever_set_entity.tsv`, but keep the zip too
5. Create a denylist containing ALL denylisted samples present in your set data table. If your set data table includes multiple instances of a denylisted sample, your denylist should contain the same number of instances (order doesn't matter). 
...
6. `python3 redact_samples.py denylist_complete.txt --set_data_table_zip whatever_set.zip`
7. You will now have three MODIFIED files which you should upload to a Terra workspace bucket:
	a) MODIFIED_bigtree_combined_YYYY-MM-DD.diff
	b) MODIFIED_whatever_set_membership.tsv
	c) MODIFIED_samples_added_YYYY-MM-DD
8. Go to your unzipped set table's unmodified `whatever_set/whatever_set_entity.tsv` file and manually modify the most recent completed Tree Nine run's values for updated_diff_file **and** updated_diff_contents to point to their respective MODIFIED files.
...
9. In Terra, delete the bucketname data table, but NOT the sample-level whatever data table
10. In Terra, upload your modified `whatever_set/whatever_set_entity.tsv`, which now has those two updated paths to point to the MODIFIED files, as a data table in the usual method
	* This will take a minute, wait for the UI to show the success in the top right corner
	* You could alternatively use FISS for this but for most users dragging-and-dropping into the UI is simpler
11. In Terra, upload your modified `MODIFIED_whatever_set_membership.tsv` file
...
12. Do a new batch of samples as usual.
